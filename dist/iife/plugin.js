 ...
 */
export const iife = (entries, options = {}) => {
    const pkg = options.pkg ?? '@pingid/vite';
    const mods = new Map(Object.entries(entries).map(([key, input]) => {
        if (key.includes('/'))
            throw new Error(`[pingid:iife] entry key ${JSON.stringify(key)} must not contain "/"`);
    ...
    return {
        ...
        configResolved: async (config_) => {
            ...
            for (const m of mods.values())
                m.file = path.resolve(config_.root, m.raw);
        ...
        configureServer: (server) => {
            ...
            server.middlewares.use((req, res, next) => {
                ...
                void build_.then((out) => {
                    res.setHeader('Content-Type', 'text/javascript; charset=utf-8');
                    res.setHeader('Cache-Control', 'no-store');
                    // Lets a non-root `fileName` still claim root scope.
                    if (route.kind === 'shell')
                        res.setHeader('Service-Worker-Allowed', config.base);
                    res.end(out.code);
                }, (e) => fail(res, e, config));
                return;
            });
 ...
 */
const wrapper = (mod, url, hot) => {
    const w = Jitt.create();
    if (hot)
        w.ln(`import { ping } from ${Jitt.str(sibling('./client.ts'))}`);
