import { create } from '../virtual/runtime.js';
...
export const boot = (mod) => void start(mod);
/**
...
export const bootHot = (mod, hot) => {
    ...
    const pull = async () => {
        ...
        if (!res.ok)
            throw new Error(`${res.status} from ${hot}`);
        const mod_ = new Function(await res.text())();
    ...
    const swap = () => {
        if (running)
            return void (again = true);
        running = pull()
            .catch((e) => console.error('[pingid:iife] hot update failed', e))
            .finally(() => {
            running = null;
            if (again)
                ((again = false), swap());
