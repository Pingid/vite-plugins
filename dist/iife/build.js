...
export const bundler = (config, shared = {}) => {
    const base = merge(opts(config), shared);
    const runtime = spec(sibling('./runtime.ts'));
    const run = async (over) => {
        const res = await esbuild.build({ ...base, ...over }).catch((e) => {
            throw new Error(`[pingid:iife] bundle failed\n${format(e)}`);
        });
        for (const w of res.warnings)
            config.logger.warn(`[pingid:iife] ${w.text}`);
        const code = res.outputFiles?.[0]?.text;
        if (code === undefined)
            throw new Error('[pingid:iife] esbuild produced no output');
...
const aliases = (config) => {
    ...
    if (dropped.length)
        config.logger.warn(`[pingid:iife] esbuild cannot honour regex aliases, so these are ignored inside the bundle: ${dropped.join(', ')}`);
    return kept;
};
const format = (e) => {
